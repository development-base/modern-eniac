...
<?php
		$alreadyin = $row['sender'];
		}
	}
	$query= "SELECT DISTINCT sender,receiver FROM messages WHERE sender='".$_SESSION['username']."'";
	$result=$con->query($query);
	if($result->num_rows > 0){
		while($row=$result->fetch_array()){
			if($alreadyin != $row['receiver']){
		?>
		<table class="mess">
		<tr><td>
		<form action="chatbox_delete.php" method="$_POST" onsubmit="return confirm('confirm delete this convo/request?')">
			<input type="hidden" name="id" value="<?php echo $row['receiver']?>">
			<input type="submit" value="&times;" class="x">
		</form></a></td>
	</tr>
	<tr><td><a href="conversation.php?sender=<?php echo $row['receiver'];?>"><img src="mail.png" width="100px" height="50px"></a></td></tr>
	<tr><td class="name"><?php echo $row['receiver']?></td></tr>
	</table>
	<?php
			}
		}
